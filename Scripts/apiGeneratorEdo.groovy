...
import org.freeplane.core.util.MenuUtils //added
import org.freeplane.core.util.TextUtils
import org.freeplane.launcher.Launcher //added
...
URI getApiLink(String path) {
    try {
        def apiBase = path.startsWith('org/freeplane') ? freeplaneApiBase
                : 'http://docs.groovy-lang.org/latest/html/groovy-jdk' + '/' //changed
        return new URI(apiBase + path) //changed
    } catch (Exception e) {
...
def typeToString(Class clazz) {
    ...
    return clazz.name.reverse().takeBefore('.').reverse().replace('Proxy$', '').replace('$', '.') //changed
}
...
def isYesOptionDialog(String title = 'Input dialog',String question = 'Please select an option') {
...
def myTitle = "Api Generator"
def myQuestion =  "Links to Api information must point to:"
String[] myOptions = ["Local Freeplane's API documentation","Freeplane's API documentation in Freeplane's webpage"]
def myResults = [new File(ResourceController.resourceController.installationBaseDir).toURI().toString() + 'doc/api','http://www.freeplane.org/doc/api/index.html?']
def resp = showInputDialogList(myOptions, myTitle, myQuestion, 0)
if (resp<0) {
    c.statusInfo = " Api Generator cancelled "
    return
}
this.freeplaneApiBase = myResults[resp]
// <- changed

// added ->
myQuestion =  "Show icons in Icons node"
def showIcons = isYesOptionDialog(myTitle,myQuestion)
// <- added
...
makeApi(utils, HtmlUtils.IndexPair.class) //added
makeApi(utils, LogUtils.class)
makeApi(utils, MenuUtils.class)
makeApi(utils, MenuUtils.MenuEntry.class) //added
makeApi(utils, MenuUtils.MenuEntryTreeBuilder.class) //added
makeApi(utils, TextUtils.class)
//org.freeplane.launcher
makeApi(utils, Launcher.class) //added
...
bundle.getKeys().toList()
    ...
    .each {
        ...
        if (showIcons) tnode.icons.add(translationAndKey[1]) //added
    }
icons.folded = true

def web = createChild(newMap.root, WEB_NODE, 'http://freeplane.sourceforge.net/wiki/index.php/Scripting')
web.details = "'Scripting' in Freeplane's wiki" // added
initHeading(web)
createChild(web, 'Groovy tutorials (Codehaus)', 'http://groovy.codehaus.org/Beginners+Tutorial') //doesn't exist anymore
